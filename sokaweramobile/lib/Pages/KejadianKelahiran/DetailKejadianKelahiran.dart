...
class DetailKejadianKelahiran extends StatefulWidget {
  const DetailKejadianKelahiran({super.key});

  @override
  State<DetailKejadianKelahiran> createState() =>
      _DetailKejadianKelahiranState();
}

class _DetailKejadianKelahiranState extends State<DetailKejadianKelahiran> {
  var nik_kk_local;
  ...
  _savedDataToLocal() async {
    ...
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    localStorage.setString('kejadian_kelahiran', userEncode);

    if (localStorage.getString('kejadian_kelahiran') != null) {
      var currentList =
          localStorage.getStringList('detail_kejadian_kelahiran') ?? [];
      currentList.add(userEncode);
      await localStorage.setStringList(
          'detail_kejadian_kelahiran', currentList);
  ...
  _deleteLocalData() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    localStorage.remove("kejadian_kelahiran");
    localStorage.remove("detail_kejadian_kelahiran");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        elevation: 2,
        centerTitle: true,
        title: Text("Detail Kejadian Kelahiran"),
